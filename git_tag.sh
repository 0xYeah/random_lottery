...
ProductName=$(grep '^name' Cargo.toml | head -1 | awk -F '"' '{print $2}')
VersionFile=Cargo.toml

# Cargo.toml stores version without 'v' prefix; script uses 'v' prefix internally
CURRENT_VERSION="v$(grep '^version' Cargo.toml | head -1 | awk -F '"' '{print $2}')"
...
function git_handle_ready() {
    ...
    local cur_no=${CURRENT_VERSION//v/}
    local next_no=${NEXT_VERSION//v/}
    sed -i -e "s/^version = \"${cur_no}\"/version = \"${next_no}\"/" $VersionFile
