...
pub fn root(app: &LotteryApp) -> Element<Message> {
    ...
    let content: Element<Message> = if let Some(err) = app.error.as_deref() {
        column![toolbar, error_banner(err), body]
            .spacing(0)
            .height(Length::Fill)
            .into()
    } else {
...
fn toolbar(app: &LotteryApp) -> Element<Message> {
...
fn prize_panel(app: &LotteryApp) -> Element<Message> {
...
fn draw_panel(app: &LotteryApp) -> Element<Message> {
    let mode_row = row![
    ...
    let btn_stop =
        ...
        .on_press_maybe(if drawing {
            Some(Message::StopDraw)
        } else {
            None
        })
        .style(move |_: &Theme, status| button::Style {
...
fn rolling_display(app: &LotteryApp) -> Element<Message> {
    let drawing = app.draw_state == DrawState::Drawing;
...
fn result_panel(app: &LotteryApp) -> Element<Message> {
...
fn error_banner(msg: &str) -> Element<Message> {
    container(
        row![
...
fn gold_button(label: &str, msg: Message, disabled: bool) -> Element<'static, Message> {
    let label = label.to_string();
    button(text(label).size(14).color(if disabled {
        c::TEXT_MUTED
    } else {
        c::BG_PRIMARY
    }))
...
fn panel_container(content: Element<Message>, min_width: u16) -> Element<Message> {
